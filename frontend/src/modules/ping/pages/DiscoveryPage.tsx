...
export function DiscoveryPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Discovery"
        description="Scan an IPv4 range, detect online hosts, and auto-save new devices"
      />
          ...
          <CardDescription>
            {hintQuery.data ? (
              <>
                ...
                <span className="mono text-foreground">{hintQuery.data.network}</span> (this machine is{' '}
                <span className="mono text-foreground">{hintQuery.data.localIP}</span>)
              </>
            ) : (
              'Scans up to 1024 addresses. Online hosts not already monitored are saved automatically.'
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={(e) => void onSubmit(e)}>
            ...
            <div className="flex flex-wrap justify-end gap-2">
              {hintQuery.data ? (
                <Button type="button" variant="secondary" disabled={scanning} onClick={applyNetworkHint}>
        ...
        <Card className="glass overflow-hidden">
          <CardContent className="space-y-4 py-8">
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <div>
                <p className="font-semibold">Scanning range</p>
      ...
      {summary ? (
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="Scanned" value={summary.totalScanned} tone="accent" />
          <KpiCard label="Online" value={summary.online} tone="success" />
          <KpiCard label="Offline" value={summary.offline} tone="danger" />
          <KpiCard label="Newly saved" value={savedCount} tone="accent" />
        </section>
      ) : null}
